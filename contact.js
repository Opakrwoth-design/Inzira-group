...
const valuesData = [
    {
        ...
        image: "/assets/img/inzirabg-2.jpg"
    },
    {
        title: "Excellence",
    ...
    return `
        <div class="relative overflow-hidden rounded-2xl cursor-pointer group" style="height: 420px;">
            ...
            <div class="absolute inset-0 flex flex-col justify-end p-6 transition-all duration-400"
                style="background: linear-gradient(to top, rgba(28,52,89,0.92) 0%, rgba(28,52,89,0.3) 60%, transparent 100%);
...
const candidateSubmitBtn = document.getElementById('candidate-submit-btn');
const candidateSuccess = document.getElementById('candidate-form-success');
const candidateError = document.getElementById('candidate-form-error');

if (candidateForm) {
    candidateForm.addEventListener('submit', async (e) => {
        e.preventDefault();

        candidateSubmitBtn.textContent = 'Sending...';
        candidateSubmitBtn.disabled = true;
        candidateSubmitBtn.classList.add('opacity-70', 'cursor-not-allowed');
        ...
        try {
            ...
            if (response.ok) {
                ...
                candidateSubmitBtn.textContent = 'Message Sent ✓';
                candidateSubmitBtn.classList.add('bg-green-600');
                candidateSubmitBtn.classList.remove('bg-[#20324A]');
            } else {
                throw new Error('Server error');
            }
        } catch (err) {
            candidateError.classList.remove('hidden');
            candidateSubmitBtn.textContent = 'Send Message';
            candidateSubmitBtn.disabled = false;
            candidateSubmitBtn.classList.remove('opacity-70', 'cursor-not-allowed');
        }
    });
}
...
const feedbackSuccess = document.getElementById('feedback-form-success');
const feedbackError = document.getElementById('feedback-form-error');
...
if (feedbackForm) {
    feedbackForm.addEventListener('submit', async (e) => {
        ...
        feedbackSubmitBtn.classList.add('opacity-70', 'cursor-not-allowed');
        ...
        try {
            ...
            if (response.ok) {
                ...
                feedbackSubmitBtn.textContent = 'Feedback Sent ✓';
                feedbackSubmitBtn.classList.add('bg-green-600');
                feedbackSubmitBtn.classList.remove('bg-[#203C47]');
            } else {
                throw new Error('Server error');
            }
        } catch (err) {
            ...
            feedbackSubmitBtn.classList.remove('opacity-70', 'cursor-not-allowed');
        }
    });
}